...
<html>
    ...
    <%
		...
		scream_bbs scream_bbs = new scream_BbsDAO().getBbs(name);
	%>
          ...
          <div class="div_box border border-info rounded-lg">
            ...
            <div class="" style="background: #ddd; position: absolute; width:100%; height:70%; top:30%">
              <div class="passwordBox rounded-lg">
                <%
                  if(which == 1){
                     %><form action="scream_update.jsp?name=<%= name %>" method="post"><%
                  }
                  else if(which == 2){
                      %><form action="scream_delete.jsp?name=<%= name %>" method="post"><%
                  }
                  %>
                
                    <input class="btn" style="background: white; width:50%;" type="password" name="password" placeholder="********">
                    <input type="hidden" name="content" value="<%= scream_bbs.getcontent() %>">
                    <input type="hidden" name="AVGtier" value="<%= scream_bbs.getAVGtier()%>">
                    <input type="hidden" name="name" value="<%= scream_bbs.getname() %>">
